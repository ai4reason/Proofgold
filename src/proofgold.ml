        ...
        try
          ...
          try
            ...
            | Some(x3,y3) -> (** shared secret **)
               ...
               let r = rand_256() in
               let sg = sign_proofgold_message (hashval_hexstring (sha256str cmsg)) k1 r in
    ...
    begin
      ...
      try
        ...
        try
          ...
          in
          let r = rand_256() in
          let sg = sign_proofgold_message (hashval_hexstring (sha256str msg)) k r in
...
let initialize_commands () =
  ...
  ac "retractltcblockandexit" "retractltcblockandexit <ltcblock>" "Purge ltc information back to the given block and exit.\nWhen Proofgold restarts it will resync with ltc back to the retracted block."
    (fun oc al ->
      ...
      | _ -> raise BadCommandForm);
  ac "sendtoaddress" "sendtoaddress <payaddress> <bars>" "Consolidate enough spendable utxos to send the given number of bars to the given payaddress"
    (fun oc al ->
      match al with
      | [a;v] ->
	 ...
	 begin
	   if p = 0 then
		 ...
		 let (k,b,_,_,_,_) = s (!Commands.walletkeys_staking @ !Commands.walletkeys_nonstaking @ !Commands.walletkeys_staking_fresh @ !Commands.walletkeys_nonstaking_fresh) in
                 let r = rand_256() in
                 let (r,s) = sign_proofgold_message m k r in
  ...
  ac "verifymessage" "verifymessage <pubkey> <signature> <msg>" "Verify the signature of the message by the key for the pubkey is valid."
    (fun oc al ->
      ...
      | _ -> raise BadCommandForm);
  ac "importid" "importid <username> <address>" "Locally associate a username with an address in the local wallet.\nThis identity is assumed to already have been registered with registerid."
    (fun oc al ->
      match al with
      | [u;a] ->
         if not (validusername_p u) then
           begin
             Printf.fprintf oc "Registered usernames must be nonempty, have at most 32 chars, and can only contain alphanumeric characters and underscores.\n";
	   ...
	   if p = 0 then
		 ...
		 let (k,b,(x,y),_,_,_) = s (!Commands.walletkeys_staking @ !Commands.walletkeys_nonstaking @ !Commands.walletkeys_staking_fresh @ !Commands.walletkeys_nonstaking_fresh) in
                 let pubkeyhex =
                   if b then
                     if evenp y then
		       Printf.sprintf "02%s" (md256_hexstring (big_int_md256 x))
		     else
		       Printf.sprintf "03%s" (md256_hexstring (big_int_md256 x))
		   else
  ...
  ac "registerid" "registerid <username> <address>" "Associate a pubkey (the one for the given address) with a username on proofgold.org.\nThis can then be used to exchange private messages (end-to-end encrypted)\nwith other registered users via the pm command.\nThe address must be a p2pkh address in the wallet.\nThe command newaddress can be used to obtain a fresh p2pkh address.\nThe commands importprivkey and importbtcprivkey can be used if you have a private key for a p2pkh address in WIF Format."
    (fun oc al ->
	  ...
	  begin
		...
		try
		  ...
		  let (k,b,(x,y),_,_,_) = s (!Commands.walletkeys_staking @ !Commands.walletkeys_nonstaking @ !Commands.walletkeys_staking_fresh @ !Commands.walletkeys_nonstaking_fresh) in
		      ...
		      Printf.sprintf "04%s%s" (md256_hexstring (big_int_md256 x)) (md256_hexstring (big_int_md256 y))
                  in
                  let msg = Printf.sprintf "registerid:u=%s:k=%s" u pubkeyhex in
                  let r = rand_256() in
                  let sg = sign_proofgold_message (hashval_hexstring (sha256str msg)) k r in
                  let sg = encode_signature 0 b sg in
                  let fullcall = Printf.sprintf "%s -X POST -F 'u=%s' -F 'k=%s' -F 's=%s' https://proofgold.org/msg/registerid.php" !Config.curl u pubkeyhex (string_hexstring sg) in
                  let (inc,outc,errc) = Unix.open_process_full fullcall [| |] in
                  begin
                    try
                      let l = input_line inc in
      ...
      | _ -> raise BadCommandForm);
  ac "getid" "getid <username>" "Try to get the pubkey associated with a username registered on proofgold.org."
    (fun oc al ->
      match al with
      | [u] ->
         if not (validusername_p u) then
         ...
         let fullcall = Printf.sprintf "%s -X POST -F 'u=%s' https://proofgold.org/msg/getid.php" !Config.curl u in
         let (inc,outc,errc) = Unix.open_process_full fullcall [| |] in
         begin
           try
             let l = input_line inc in
             try
               let ((x,y),b) = Cryptocurr.hexstring_pubkey l in
               let alpha1 = Cryptocurr.pubkey_md160 (x,y) b in
             ...
             with _ ->
                   begin
                     try
                       Printf.fprintf oc "Failed to get identity info.\n%s\n" l;
                       while true do
                         let l = input_line inc in
                         Printf.fprintf oc "%s\n" l;
                         flush oc
           ...
           with _ -> Printf.fprintf oc "Failed to get identity info.\n"
         end
      | _ -> raise BadCommandForm);
  ac "getmessages" "getmessages <user> [<timestamp>]" "Get all private messages for the user since the given timestamp.\nIf no timestamp is given, all messages from the past week are downloaded."
    (fun oc al ->
      let currtm = Int64.sub (Int64.of_float (Unix.time())) 300L in (** allow for the clock to be 5 minutes off from the server **)
      let (u,sincetm) =
        match al with
        | [u] -> (u,Int64.sub currtm 604800L)
        | [u;sincetm] -> (u,Int64.of_string sincetm)
        | _ -> raise BadCommandForm
      in
      load_identities();
      try
        let (b,k,_,_) = Hashtbl.find identities u in
        let msg = Printf.sprintf "getmessages:u=%s:t=%Ld" u sincetm in
        let r = rand_256() in
        let sg = sign_proofgold_message (hashval_hexstring (sha256str msg)) k r in
        let sg = encode_signature 0 b sg in
        let fullcall = Printf.sprintf "%s -X POST -F 'u=%s' -F 't=%Ld' -F 's=%s' https://proofgold.org/msg/getmessages.php" !Config.curl u sincetm (string_hexstring sg) in
        let (inc,outc,errc) = Unix.open_process_full fullcall [| |] in
         begin
           try
             let l = input_line inc in
             if l = "OK" then
               while true do
                 ...
                 try
                   ...
                   try
                     match Secp256k1.smulp k (Some(x,y)) with
                     | Some(x3,y3) -> (** shared secret **)
                        let emsg = hexstring_string emsghex in
                        let emsgl = String.length emsg in
                        if emsgl < 34 then
                          Printf.fprintf oc "Encrypted message is too short. Something went wrong.\n"
                        else
                          let msgl = (Char.code emsg.[0]) * 256 + Char.code emsg.[1] in
                          if emsgl = 34 + msgl then
                            begin
                              Printf.fprintf oc "\n=========\n";
                              let xs = hexstring_string (hashval_hexstring (big_int_md256 x3)) in
                              let padseedb = Buffer.create 32 in
                              for i = 0 to 31 do Buffer.add_char padseedb (Char.chr ((Char.code xs.[i]) lxor (Char.code emsg.[i+2]))) done;
                              let padseedh = hexstring_hashval (string_hexstring (Buffer.contents padseedb)) in
                              let tg = ref 0 in
                              let pad = ref (hexstring_string (hashval_hexstring (hashtag padseedh (Int32.of_int !tg)))) in
                              let j = ref 0 in
                              for i = 0 to msgl -1 do
                                Printf.fprintf oc "%c" (Char.chr ((Char.code !pad.[!j]) lxor (Char.code emsg.[i+34])));
                                incr j;
                                if !j = 32 then
                                  begin
                                    incr tg;
                            ...
                            Printf.fprintf oc "Message length mismatch. Something went wrong.\n"
                     | None ->
                        Printf.fprintf oc "Due to extremely rare null point, these two users cannot share private messages.\nThis is probably a bug.\n"
                   with _ ->
                     Printf.fprintf oc "Problem trying to decrypt private message.\n"
                 with
                 | Not_found ->
                    Printf.fprintf oc "Do not have public key for %s, so cannot decrypt message.\nUse getid first, and then use getmessages again.\n" fromuser
                 | _ -> ()
               done
             else if (l = "NONE") then
           ...
           with _ ->
             ignore (Unix.close_process_full (inc,outc,errc));
             Printf.fprintf oc "Now have messages until time %Ld.\nNext time call:\ngetmessages %s %Ld\n" currtm u currtm
         end
      with Not_found ->
        Printf.fprintf oc "Do not have private key for %s so not downloading messages.\n" u);
  ac "savemessages" "savemessages <user> <file> [<timestamp>]" "Get all private messages for the user since the given timestamp and save them into the given file.\nIf no timestamp is given, all messages from the past week are downloaded."
    (fun oc al ->
      let currtm = Int64.sub (Int64.of_float (Unix.time())) 300L in (** allow for the clock to be 5 minutes off from the server **)
      let (u,fn,sincetm) =
        match al with
        | [u;fn] -> (u,fn,Int64.sub currtm 604800L)
        | [u;fn;sincetm] -> (u,fn,Int64.of_string sincetm)
        | _ -> raise BadCommandForm
      in
      load_identities();
      try
                 ...
                 try
                   ...
                   try
                     ...
                     | Some(x3,y3) -> (** shared secret **)
                        ...
                        if emsgl < 34 then
                          begin
                            Printf.fprintf f "Encrypted message is too short. Something went wrong.\n";
                            close_out f
                          end
                        else
                          let msgl = (Char.code emsg.[0]) * 256 + Char.code emsg.[1] in
                          if emsgl = 34 + msgl then
                            begin
                              ...
                              let j = ref 0 in
                              for i = 0 to msgl -1 do
                                Printf.fprintf f "%c" (Char.chr ((Char.code !pad.[!j]) lxor (Char.code emsg.[i+34])));
                                incr j;
                                if !j = 32 then
                                  begin
                                    incr tg;
                            ...
                            Printf.fprintf f "Message length mismatch. Something went wrong.\n";
                          close_out f
                     | None ->
                        Printf.fprintf f "Due to extremely rare null point, these two users cannot share private messages.\nThis is probably a bug.\n";
                        close_out f;
                   with _ ->
                     Printf.fprintf f "Problem trying to decrypt private message.\n";
                     close_out f;
                 with
                 | Not_found ->
                    Printf.fprintf f "Do not have public key for %s, so cannot decrypt message.\nUse getid first, and then use savemessages again.\n" fromuser;
                    close_out f;
                 | _ -> ()
               done             
             else if (l = "NONE") then
      ...
      with Not_found ->
        Printf.fprintf oc "Do not have private key for %s so not downloading messages.\n" u);
  ac "pm" "pm <fromuser> <touser> <text>" "Send the (short) text as a private message."
    (fun oc al ->
      match al with
      | [fromuser;touser;msg] ->
         sendprivmessage oc fromuser touser msg
      | (fromuser::touser::msg::msgr) ->
         let sb = Buffer.create 100 in
         Buffer.add_string sb msg;
         List.iter (fun s -> Buffer.add_char sb ' '; Buffer.add_string sb s) msgr;
         sendprivmessage oc fromuser touser (Buffer.contents sb)
      | _ -> raise BadCommandForm);
  ac "pmfile" "pmfile <fromuser> <touser> <filename>" "Send the contents of the given text file as a private message."
    (fun oc al ->
      match al with
      | [fromuser;touser;fn] ->
         let f = open_in fn in
      ...
      | _ -> raise BadCommandForm);
  ac "posttop" "posttop <user> <text>" "Post in the top level of the latest anon topic on the proofgold forum.\nThe given username is assumed to be registered with registerid."
    (fun oc al ->
      match al with
      | [u;p] -> apost oc u p None
      | (u::p::pr) ->
         let sb = Buffer.create 100 in
         Buffer.add_string sb p;
         List.iter (fun s -> Buffer.add_char sb ' '; Buffer.add_string sb s) pr;
         apost oc u (Buffer.contents sb) None
      | _ -> raise BadCommandForm);
  ac "postreply" "postreply <user> <parentid> <text>" "Post reply on the proofgold forum"
    (fun oc al ->
      match al with
      | [u;pid;p] -> apost oc u p (Some(pid))
      | (u::pid::p::pr) ->
         let sb = Buffer.create 100 in
         Buffer.add_string sb p;
         List.iter (fun s -> Buffer.add_char sb ' '; Buffer.add_string sb s) pr;
         apost oc u (Buffer.contents sb) (Some(pid))
      | _ -> raise BadCommandForm);
	  ...
	  if l = "Theory" then
		...
		if th1 = th then
		  ...
		  Hashtbl.iter
		    ...
		    bountyh;
		  if !countbounties > 0 then Printf.printf "%d new bounties worth a total of %s bars.\n" !countbounties (bars_of_atoms !totalbounties)
	    end
	  else
	    begin
	      close_in ch;
