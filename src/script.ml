	...
	begin
	  match stk with
          | (mtxid::mc::stkr) ->
             ...
             let c = num32 mc in
             begin
               match ltc_tx_confirmed txid with
               | Some(c2) when Int32.of_int c2 >= c ->
                  eval_script_r tosign br stkr altstk
               | _ ->
                  raise Invalid
             end
    ...
    | (180::br) -> (** OP_LBH_VERIFY blkid h -- check if blkid is the block id for litecoin tx at height h; if yes, continue, otherwise fail -- was OP_NOP5 in bitcoin **)
	  ...
	  match stk with
          | (mblkid::mh::stkr) ->
             ...
             begin
               try
                 if ltc_getblockheight blkid = Int64.of_int32 h then
                   eval_script_r tosign br stkr altstk
                 else
                   raise Invalid
               with Not_found ->
...
(*** eval_script_r is mutually recursive with checksig and checkmultisig since endorsements require scripts to be evaluated to check the signatures of endorsees ***)
  and checksig tosign gsg pubkey =
    try
		...
		begin
		  ...
		  | Some(x2,y2) ->
			...
			if c = 4 then
		      ...
		      let ee = md256_big_int (md256_of_bitcoin_message ("endorse " ^ (addr_pfgaddrstr (md160_p2pkh_addr beta)))) in
	  ...
	  (*** alpha signs that beta can sign ***)
	  let ee = md256_big_int (md256_of_bitcoin_message ("endorse " ^ (addr_pfgaddrstr (md160_p2sh_addr beta)))) in
...
let verify_gensignat obday e gsg alpha =
  ...
  | EndP2pkhToP2pkhSignat(Some(x,y),c,Some(w,z),d,esg,sg) ->
	...
	if ok then
	  ...
	  let ee = md256_big_int (md256_of_bitcoin_message ("fakeendorsement " ^ (addr_pfgaddrstr (md160_p2pkh_addr beta)) ^ " (" ^ (addr_pfgaddrstr alpha) ^ ")")) in
...
let json_gensignat gs =
  ...
  | EndP2pkhToP2shSignat(p,c,beta,esg,scr) ->
      JsonObj([("type",JsonStr("gensignat"));("gensignattype",JsonStr("endp2pkhtop2sh"));
	       ("pubkey1",json_pt p);("compressed1",JsonBool(c));
	       ("p2sh",JsonStr(addr_pfgaddrstr (md160_p2sh_addr beta)));
