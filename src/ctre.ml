...
let rec print_ctree_all_r f blkh c n br =
  ...
  | CLeaf(bl,hl) ->
	...
	Printf.fprintf f "Leaf %s\n" (addr_pfgaddrstr (bitseq_addr ((List.rev br) @ bl)))
      else
    ...
    (fun (alpha,(obl,u)) ->
	  ...
	  begin
	    if not (termaddr_addr (hashval_md160 oid) = alpha) then
	      begin
		vmsg (fun oc -> Printf.fprintf oc "OwnsObj %s should be sent to address %s not %s.\n" (hashval_hexstring oid) (Cryptocurr.addr_pfgaddrstr (termaddr_addr (hashval_md160 oid))) (Cryptocurr.addr_pfgaddrstr alpha));
	  ...
	  begin
	    if not (termaddr_addr (hashval_md160 pid) = alpha) then
	      begin
		vmsg (fun oc -> Printf.fprintf oc "OwnsProp %s should be sent to address %s not %s.\n" (hashval_hexstring pid) (Cryptocurr.addr_pfgaddrstr (termaddr_addr (hashval_md160 pid))) (Cryptocurr.addr_pfgaddrstr alpha));
