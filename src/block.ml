  ...
  if (try DbInvalidatedBlocks.dbget bhdh with Not_found -> false) then (*** explicitly marked as invalid ***)
    ...
    begin
	  ...
	  || (!Config.testnet (*** allow fake endorsements in testnet ***)
		&&
	      verifybitcoinmessage (-916116462l, -1122756662l, 602820575l, 669938289l, 1956032577l) recid fcomp esg ("fakeendorsement " ^ (addr_pfgaddrstr (p2pkhaddr_addr beta)) ^ " (" ^ (addr_pfgaddrstr (p2pkhaddr_addr bhd.stakeaddr)) ^ ")")
		&&
	      verify_p2pkhaddr_signat (hashval_big_int bhdh) beta bhs.blocksignat bhs.blocksignatrecid bhs.blocksignatfcomp)
	  end
    end
...
let rec valid_blockchain_aux blkh bl lmedtm burned txid1 vout1 =
  match bl with
  | ((bh,bd)::(pbh,pbd)::br) ->
      if blkh > 1L then
	begin
	  ...
	  | Some(bhprev,(Poburn(lblk,ltx,lmedtm1,burned1,txid2,vout2) as pob)) ->
	     let (tht,sigt) = valid_blockchain_aux (Int64.sub blkh 1L) ((pbh,pbd)::br) lmedtm1 burned1 txid2 vout2 in
	     let csm = poburn_stakemod pob in
	     if blockheader_succ pbh bh then
		begin
		  match valid_block tht sigt blkh csm pbhd.tinfo (bh,bd) lmedtm burned txid1 vout1 with
  ...
  | [(bh,bd)] ->
      ...
      then
	begin
	  match valid_block None None blkh !genesisstakemod !genesistarget (bh,bd) lmedtm burned txid1 vout1 with
	  | Some(tht2,sigt2) -> (tht2,sigt2)
	  | None -> raise NotSupported
	end
      else
...
let valid_blockchain blkh bc lmedtm burned txid1 vout1 =
  try
    let (b,bl) = bc in
    ignore (valid_blockchain_aux blkh (b::bl) lmedtm burned txid1 vout1);
    true
  with NotSupported -> false

let rec valid_blockheaderchain_aux blkh bhl lmedtm burned txid1 vout1 =
  match bhl with
  | (bh::pbh::bhr) ->
      if blkh > 1L then
	begin
	  ...
	  | Some(bhprev,(Poburn(lblk,ltx,lmedtm1,burned1,txid2,vout2) as pob)) ->
	     let csm = poburn_stakemod pob in
	     valid_blockheaderchain_aux (Int64.sub blkh 1L) (pbh::bhr) lmedtm1 burned1 txid2 vout2
	     && blockheader_succ pbh bh
	     && valid_blockheader blkh csm pbhd.tinfo bh lmedtm burned txid1 vout1
	end
      else
	false
  | [(bhd,bhs)] ->
      blkh = 1L
	&&
      valid_blockheader blkh !genesisstakemod !genesistarget (bhd,bhs) lmedtm burned txid1 vout1
	&&
